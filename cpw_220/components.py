...
def calc_line_intersect(a1, a2, b1, b2, angles = False, ret_points = False):
    """
    Calculates and return the xy point where two line segments intersect.
    ...
    """
    if angles == True:
        # calculate points a2, b2 from angle
        a2 = a1 + np.array([ np.sin(np.deg2rad(a2)), np.cos(np.deg2rad(a2)) ])
        b2 = b1 + np.array([ np.sin(np.deg2rad(b2)), np.cos(np.deg2rad(b2)) ])
    ...
    if ret_points:
        return (x/z, y/z), np.array([a1,a2,b1,b2])
    else:
        return (x/z, y/z)
...
def x_idc_to_abs_path_2(x_abs_pars, x_rcap_pars, x_abs_ref, x_rcap_ref, layer=None):
    ...
    radius_separation = 26*x_abs_pars["lw"]
...
def x_rcap_to_port(x_rcap_ref, x_rcap_pars, layer=None):

    points=[]

    radius_tolerance = 10*x_rcap_pars["tine_linewidth"]
    arc_radius = x_rcap_pars["outer_radius"] + x_rcap_pars["rail_extend_out"] + radius_tolerance
    points.append([0, arc_radius])
...
def y_rcap_to_port(y_rcap_ref, y_rcap_pars, y_ccap_pars, layer=None):
...
def optical_chip_outline(dev_design, dev_label, chip_pars):
    ...
    #dashed line of tabs
    tab = phidl.Device(name='tab')
    dl = 3800
    points = [(0,0), (0,dl)]
    dash = polypath_from_points(xypoints=points, lw=w, layer=chip_pars['line_layer'], corners='smooth')
    dot = geo.circle(radius=w/2., angle_resolution=2, layer=chip_pars['line_layer'])
    tab.add_ref(dash)
    tab.add_ref(dot).movey(dl)
    tab.add_ref(dot)
    

    upper=outline.add_ref(horizontal).movex(-l/2.).movey(l/2.-o)
    #left dashed line
    #outline.add_ref(tab).movex(-l/2.+o-w).movey(-l/2.)
    #outline.add_ref(tab).movex(-l/2.+o-w).movey(-l/2.+dl+500)
    #outline.add_ref(tab).movex(-l/2.+o-w).movey(-l/2.+2*dl+1000)
    #outline.add_ref(tab).movex(-l/2.+o-w).movey(-l/2.+3*dl+1500)
    #outline.add_ref(tab).movex(-l/2.+o-w).movey(-l/2.+4*dl+2000)
    #outline.add_ref(tab).movex(-l/2.+o-w).movey(-l/2.+5*dl+2500)
    ...
    label = geo.text(dev_design, size=500, layer=chip_pars["text_layer"])
...
def optical_chip_outline_notext(chip_pars):
...
def dark_chip_outline(dev_label, chip_pars):
    ...
    #dot = geo.circle(radius=w/2., angle_resolution=2, layer=chip_pars['line_layer'])
    #tab.add_ref(dash)
    #tab.add_ref(dot).movex(dl)
    #tab.add_ref(dot)
    

    #upper dashed line
    #outline.add_ref(tab).movex(-sl/2.).movey(sw/2.-o)
    #outline.add_ref(tab).movex(-sl/2.+dl+500).movey(sw/2.-o)
    #outline.add_ref(tab).movex(-sl/2.+2*dl+1000).movey(sw/2.-o)
    #outline.add_ref(tab).movex(-sl/2.+3*dl+1500).movey(sw/2.-o)
    #outline.add_ref(tab).movex(-sl/2.+4*dl+2000).movey(sw/2.-o)
    #outline.add_ref(tab).movex(-sl/2.+5*dl+2500).movey(sw/2.-o)
    ...
    #text
    label_offset=chip_pars["chip_sidelength"]/2.-chip_pars["outline_offset"]-chip_pars["outline_width"]-100
    label = geo.text("SPT4 TLSv2", size=500, layer=chip_pars["text_layer"])
